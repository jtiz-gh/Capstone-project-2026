...
export async function POST(request: Request) {
  try {
    ...
    if (!data.serialNo || !data.readings || !data.eventId || !data.competitionId || !data.sessionId) {
    ...
    if (data.readings.length > 0) {
      ...
      const energy = avgVoltage && avgCurrent ? avgVoltage * avgCurrent * data.readings.length / 3600 : null;
