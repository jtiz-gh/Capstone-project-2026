...
async function main() {
  ...
  await prisma.deviceConfig.createMany({
    data: [
      ...
      { deviceId: device4.id, settings: { maxSpeed: 40, throttleCap: 80 }, configName: 'Standard Config' },
      { deviceId: device5.id, settings: { maxSpeed: 35, throttleCap: 70 }, configName: 'Conservative Config' },
      { deviceId: device6.id, settings: { maxSpeed: 45, throttleCap: 90 }, configName: 'Aggressive Config' },
    ]
  })
...
let offset = 1000
for (const [device, sessionId] of [[device1, 1], [device2, 2], [device3, 3], [device4, 4], [device5, 5], [device6, 6]] as const) {
